...
exports.gptmakerWebhook = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const ref = await db.collection('gptmaker_eventos').add({
      evento: ev,
      ...
      achouIdentificador: achado.identificador || '',
      achouCanal: achado.canal || '',
      // Campos confirmados no payload real do GPT Maker (20/08/2026):
      // agentId, interactionId, protocol, name, recipient, channel,
      // contextId (= channelId + "-" + recipient), channelId.
      // Repare no que NÃO vem: chatId e o texto da conversa.
      protocolo: (req.body && req.body.protocol) || '',
      interactionId: (req.body && req.body.interactionId) || '',
      contextId: (req.body && req.body.contextId) || '',
      previaCampos: JSON.stringify(previa.campos || {}).slice(0, 4000),
      previaExtras: JSON.stringify(previa.extras || {}).slice(0, 2000),
    });

    // Responde primeiro: webhook que demora vira reenvio em loop.
    res.status(200).json({ ok: true });

    // ── ENSAIO DA FASE 3 ────────────────────────────────────────────────────
    // O evento não traz a conversa, só identificadores. Então aqui a gente
    // TENTA fazer o que a fase 3 vai fazer de verdade: achar o chat pelo
    // recipient e puxar as mensagens. O resultado fica gravado no próprio
    // evento, para você ver na tela se funcionou — sem ainda criar lead nenhum.
    // Se falhar, falha em silêncio: é ensaio, não pode derrubar o espião.
    try {
      const alvo = String((req.body && req.body.recipient) || achado.telefone || '');
      if (alvo) await ensaiarBuscaDaConversa(ref, alvo);
    } catch (e) { console.warn('[gptmaker] ensaio falhou:', e.message); }
  } catch (err) {
...
// Vai atrás da conversa que o evento NÃO mandou.
//
// O webhook entrega `recipient` (no WhatsApp, o número) mas não o `chatId`, e
// é o chatId que a API pede para listar mensagens. Então o caminho é: listar
// os chats do workspace, achar o que bate com o recipient, e aí sim puxar o
// histórico. Como o formato do objeto chat ainda não está confirmado, a busca
// compara vários campos possíveis em vez de apostar em um.
async function ensaiarBuscaDaConversa(ref, recipient) {
  const cfg = await configGptmaker().catch(() => null);
  if (!cfg || !cfg.workspaceId) return;

  const soDigitos = v => String(v || '').replace(/\D/g, '');
  const alvo = soDigitos(recipient);
  const bate = c => {
    for (const k of ['recipient', 'whatsappPhone', 'phone', 'contextId', 'name']) {
      const v = soDigitos(c && c[k]);
      // Compara pelos 8 últimos: o 9 do celular e o DDI aparecem e somem.
      if (v && alvo && v.slice(-8) === alvo.slice(-8)) return true;
    }
    return false;
  };
  const comoLista = d => {
    if (Array.isArray(d)) return d;
    for (const k of ['data', 'content', 'items', 'results', 'chats', 'messages'])
      if (Array.isArray(d && d[k])) return d[k];
    return [];
  };

  const rChats = await chamarGptmaker({
    token: cfg.token,
    path: `/v2/workspace/${encodeURIComponent(cfg.workspaceId)}/chats?page=1&pageSize=40`,
  });
  const chats = comoLista(rChats.data);
  const chat = chats.find(bate) || null;

  const ensaio = {
    buscouEm: new Date().toISOString(),
    chatsVistos: chats.length,
    achouChat: !!chat,
    chatId: chat ? String(chat.id || '') : '',
    chatCru: chat ? JSON.stringify(chat).slice(0, 3000) : '',
    mensagens: 0,
    campos: '{}',
    texto: '',
    erro: '',
  ...
  if (!chat || !chat.id) {
    ensaio.erro = chats.length
      ? `Nenhum dos ${chats.length} chats bate com ${alvo}.`
      : 'A lista de chats voltou vazia.';
    await ref.set({ ensaio }, { merge: true });
    return;
  }

  const rMsgs = await chamarGptmaker({
    token: cfg.token,
    path: `/v2/chat/${encodeURIComponent(chat.id)}/messages?page=1&pageSize=30`,
  });
  const msgs = comoLista(rMsgs.data);
  const texto = msgs.map(m => String((m && m.text) || '')).filter(Boolean).join('\n');

  ensaio.mensagens = msgs.length;
  ensaio.texto = texto.slice(0, 4000);
  // É aqui que o formulário da Meta aparece: ele vem como PRIMEIRA MENSAGEM da
  // conversa, não no webhook. Por isso o ensaio precisa chegar até este ponto.
  ensaio.campos = JSON.stringify(lerFormularioMeta(texto).campos || {}).slice(0, 3000);
...
function farejar(obj) {
  const achado = { chatId: '', telefone: '', nome: '', identificador: '', canal: '' };
  const visto = new Set();
  (function anda(o, prof) {
    ...
    for (const [k, v] of Object.entries(o)) {
      ...
      // CUIDADO: `recipient` NÃO é telefone. A documentação chama de
      // "identificador único do chat dentro do canal" — no WhatsApp calha de
      // ser o número, mas no Instagram e no Messenger vem um id da plataforma,
      // tipo 569948728797695, que tem 15 dígitos e não disca em lugar nenhum.
      // Se a gente deduplicar lead por isso, junta gente diferente no mesmo
      // cadastro. Por isso só aceita o que tem cara de telefone brasileiro.
      if (/(phone|telefone|whats|celular|recipient)/.test(chave)) {
        const dig = txt.replace(/\D/g, '');
        const pareceTelefone =
          (dig.length >= 10 && dig.length <= 11) ||               // sem país
          (dig.length >= 12 && dig.length <= 13 && dig.startsWith('55')); // com o 55
        if (pareceTelefone) { if (!achado.telefone) achado.telefone = dig; }
        else if (dig.length > 8 && !achado.identificador) achado.identificador = dig;
      }
      if (!achado.canal && /^(channel|canal|platform|plataforma|type|conversationtype|origin)$/.test(chave)) {
        achado.canal = txt.slice(0, 30);
      }
